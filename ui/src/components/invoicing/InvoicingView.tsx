...
import { str, num, bool, entity as subEntity, list as entityList, formatDate, invoiceStatus, deepStr, isReminder, isDeposit, isFinalInvoice, displaysAsFinal, isSettlementDeposit, reminderLevel, depositChainHeadId, depositMilestoneLabel, milestoneScheduleStatus, type MilestoneScheduleStatus } from "../../api/entity";
...
function CreateInvoiceDialog({ onClose, onCreated }: { onClose: () => void; onCreated: (newId?: number, warning?: string | null) => Promise<void> | void }) {
  ...
  const [docType, setDocType] = useState<"invoice" | "deposit" | "final">("invoice");
  const [selectedMilestoneId, setSelectedMilestoneId] = useState<number | null>(null);

  const selectedProject = projects.find((p) => p.id === projectId) ?? null;
  const isFixedPrice = selectedProject ? bool(selectedProject, "is_fixed_price") : false;
  const contractEntity = selectedProject ? subEntity(selectedProject, "contract") : null;
  const milestones = contractEntity ? entityList(contractEntity, "payment_milestones") : [];
  ...
  useEffect(() => {
    if (hasMilestones && docType === "final") setDocType("deposit");
  }, [hasMilestones, docType]);
      ...
      <div className="bg-bg-content rounded-xl border border-border-subtle shadow-2xl w-[560px] max-h-[90vh] overflow-y-auto"
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                {(["invoice", "deposit"] as const).map((dt) => (
                  <button key={dt} type="button" onClick={() => { setDocType(dt); setSelectedMilestoneId(null); }}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-medium transition-colors border
                      ${docType === dt ? "border-accent bg-accent/15 text-primary" : "border-border-subtle text-tertiary"}`}>
                    {dt === "invoice" ? <><FileText size={14} /> Invoice</> :
        ...
        <div className="px-5 py-3 border-t border-border-subtle flex justify-end gap-2">
          ...
          <button onClick={submit} disabled={submitting}
            ...
            {submitting ? "Creating…" : docType === "deposit"
              ? (isLastOpenMilestone ? "Create Final Invoice" : "Create Deposit Invoice")
              : "Create Invoice"}
          </button>
        </div>
      </div>
...
function MilestoneScheduleBadge({ schedule }: { schedule: MilestoneScheduleStatus }) {
  ...
  if (allInvoiced) {
    return (
      <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 shrink-0">
        {invoicedCount}/{total} invoiced · {paidCount}/{total} paid
      </span>
    );
  }
  return (
    <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 shrink-0">
    ...
    <button onClick={onSelect}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {isDeposit(invoice) && !settlement && depositLabel && (
            <span className="text-xs font-semibold text-blue-400 truncate">{depositLabel}</span>
          )}
          {showAsFinal && (
            <>
              {depositLabel && (
                <span className="text-xs font-semibold text-blue-300 truncate">{depositLabel}</span>
              )}
              {!depositLabel && (
          ...
          {isDeposit(invoice) && !settlement && <DocumentTypeBadge type="deposit" />}
          {showAsFinal && <DocumentTypeBadge type="final" />}
...
function DepositRow({ invoice, isSelected, onSelect }: { invoice: Entity; isSelected: boolean; onSelect: () => void }) {
  ...
  return (
    <button onClick={onSelect}
      className={`w-full text-left pl-10 pr-4 py-2.5 border-b transition-colors border-l-2
        ${settlement ? "border-l-blue-400" : "border-l-blue-400"}
        ${isSelected ? "bg-bg-selected border-b-border-subtle" : "border-b-border-subtle hover:bg-bg-hover bg-bg-content/50"}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          <span className={`text-xs font-semibold truncate ${settlement ? "text-blue-300" : "text-blue-400"}`}>
            {depositLabel || (settlement ? "Final settlement" : "Deposit")}
          </span>
          {settlement ? <DocumentTypeBadge type="final" /> : <DocumentTypeBadge type="deposit" />}
...
function InvoiceChainCard({ chain, color, reminderCount, depositCount }: {
  ...
  const isFinal = isFinalInvoice(root);
  ...
  return (
    <div className="space-y-1.5">
      ...
      {deposits.length > 0 && (
        <div className={`ml-2 pl-2 border-l-2 space-y-1.5 ${isFinal || schedule?.completeWithoutFinal ? "border-blue-400/60" : "border-blue-400/60"}`}>
          {deposits.map((dep) => {
            const depositLabel = depositMilestoneLabel(dep);
            const settlement = isSettlementDeposit(dep);
            return (
          ...
          <div>
            <h1 className="text-lg font-semibold">
              {isRem
                ...
                : isSettlementDeposit(invoice)
                ? (depositLabel ? `Final · ${depositLabel}` : `Final ${str(invoice, "number") || "Draft"}`)
                : isDeposit(invoice)
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {(isDeposit(invoice) || isSettlementDeposit(invoice)) && depositLabel && (
      ...
      {detailTab === "invoice" ? (
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-5">
          ...
          {(() => {
            ...
            const paidDeposits = deposits.filter((d) => invoiceStatus(d) === "Paid").length;
            const allMilestonesInvoiced = milestones.every((m) => bool(m, "invoiced"));
            const allDepositsPaid = paidDeposits === milestones.length;
            const completeWithoutFinal =
              allMilestonesInvoiced && allDepositsPaid && !finalInv;
              ...
              <Section title="Payment Schedule">
                <div className="space-y-1">
                  {milestones.map((m) => {
                    ...
                    const depStatus = dep ? invoiceStatus(dep) : null;
                    const settlement = dep ? isSettlementDeposit(dep) : false;
                    const state = !dep && !bool(m, "invoiced")
                      ? "open"
                      : depStatus === "Paid"
                      ? "paid"
                      : dep
                      ...
                      : "invoiced";
                    return (
                      <div key={m.id} className={`flex items-center justify-between px-3 py-2 rounded-md text-xs border
                        ${settlement ? "bg-blue-500/5 border-blue-500/20" : "bg-bg-card border-border-subtle"}`}>
                        <div className="flex items-center gap-2 min-w-0">
                          <Milestone size={12} className={`shrink-0 ${settlement ? "text-blue-400" : "text-tertiary"}`} />
                          <span className="font-medium truncate">{str(m, "title") || "Untitled"}</span>
                          {settlement && (
                            <span className="text-[10px] font-medium text-blue-300 shrink-0">Final settlement</span>
                          )}
                          {dep && (
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {state === "paid" && (
                            <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-green-500/10 text-green-400">Paid</span>
                          )}
                        </div>
                      </div>
                    );
                  ...
                  {completeWithoutFinal && (
                    <div className="px-3 py-2 rounded-md text-xs bg-green-500/10 border border-green-500/20 text-green-400">
